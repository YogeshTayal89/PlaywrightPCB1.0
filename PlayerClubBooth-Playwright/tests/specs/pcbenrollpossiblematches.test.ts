...
import PCBEnrollPossibleMatchesPage from "../pages/pcbenrollpossiblematches.page";
...
test.describe('PCB Enroll Possible Matches Suite', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await test.step('Then I can click on Next button from preview page', async () => {
            ...
            await pcbenrollpreviewpage.clickOnNextButton();
        });
    });
    

    test('Scenario Outline: As a user , I can navigate to pcb possible matches page @PossibleMatches @Smoke @Regression', async ({ page }) => {
        await test.step('Then I should see PCB enrollment possible matches Page', async () => {
            const pcbenrollpossiblematches = new PCBEnrollPossibleMatchesPage(page);
            await pcbenrollpossiblematches.verifyEnrollPossibleMatchesPageIsPresent();
        });

    });

    test('Scenario Outline: As a user , I can click on the save button in possible matches page @PossibleMatches @Regression', async ({ page }) => {
        const pcbenrollpossiblematchespage = new PCBEnrollPossibleMatchesPage(page)
        await test.step('Then I should see PCB enrollment possible matches Page', async () => {
            await pcbenrollpossiblematchespage.verifyEnrollPossibleMatchesPageIsPresent();
        });

        await test.step('When I can click on save button', async () => {
            await pcbenrollpossiblematchespage.clickOnSaveButton(username,password);
        });

        await test.step('Then it should show print dialog', async () => {
